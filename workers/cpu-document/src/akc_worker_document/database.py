"""Restricted database boundary for the native document worker."""
...
class AnalysisDatabasePrivilegeError(RuntimeError):
    """Raised when the worker login has more or less authority than expected."""


@dataclass(frozen=True, slots=True)
class AnalysisDatabaseCapability:
    backend: str
...
_POSTGRES_CAPABILITY_QUERY = text(
    ...
    SELECT
        ...
        (
            has_table_privilege(current_user, 'public.analysis_tasks', 'SELECT')
            AND NOT has_table_privilege(
                current_user, 'public.analysis_tasks', 'INSERT,DELETE,TRUNCATE'
            )
        ) AS task_table_access,
        (
            SELECT bool_and(
                has_column_privilege(
                    current_user, 'public.analysis_tasks', column_name, 'UPDATE'
                )
            )
            FROM unnest(
                ARRAY[
                    ...
                    'page_count', 'block_count', 'preview_count',
        ...
        (
            ...
            AND NOT has_table_privilege(
                current_user, 'public.outbox_events', 'INSERT,DELETE,TRUNCATE'
            )
        ) AS outbox_table_access,
        (
            SELECT bool_and(
                has_column_privilege(
                    current_user, 'public.outbox_events', column_name, 'UPDATE'
                )
            )
            FROM unnest(
                ARRAY[
        ...
        (
            ...
            FROM unnest(
                ARRAY['document_type', 'page_count', 'status', 'updated_at']
            ) AS columns(column_name)
        ...
        (
            ...
            FROM unnest(
                ARRAY['pages', 'page_assets', 'blocks', 'review_items']
            ) AS tables(table_name)
        ) AS result_table_access,
        (
            has_table_privilege(
                current_user, 'public.page_attempts', 'SELECT,INSERT'
            )
            AND NOT has_table_privilege(
                current_user, 'public.page_attempts', 'DELETE,TRUNCATE'
            )
            AND has_table_privilege(
        ...
        (
            ...
            WHERE namespace.nspname = 'public'
              AND class.relname IN (
                  'analysis_tasks', 'outbox_events', 'documents',
                  'source_files', 'pages', 'page_assets', 'blocks',
                  'review_items', 'audit_events', 'projects', 'tenants',
                  'feature_flags', 'page_attempts',
...
def create_analysis_engine(settings: AnalysisWorkerSettings) -> AsyncEngine:
    ...
    if settings.database_backend == "postgresql":
        ...
        connect_args["server_settings"] = {
            "application_name": "akc-analysis-worker",
            "role": settings.analysis_database_role,
...
async def verify_analysis_database(
    engine: AsyncEngine,
    settings: AnalysisWorkerSettings,
) -> AnalysisDatabaseCapability:
    if settings.database_backend == "sqlite":
        if settings.env == "production":
            raise AnalysisDatabasePrivilegeError("sqlite_analysis_adapter_forbidden_in_production")
        async with engine.connect() as connection:
            await connection.execute(text("SELECT 1"))
            foreign_keys = await connection.scalar(text("PRAGMA foreign_keys"))
        if foreign_keys != 1:
            raise AnalysisDatabasePrivilegeError("sqlite_foreign_keys_disabled")
        return AnalysisDatabaseCapability(
            backend="sqlite",
            effective_role=None,
            login_role=None,
            bypass_rls=False,
    ...
    if settings.database_backend != "postgresql":
        raise AnalysisDatabasePrivilegeError("unsupported_analysis_database")
    async with engine.connect() as connection:
        row = (await connection.execute(_POSTGRES_CAPABILITY_QUERY)).mappings().one_or_none()
    if row is None:
        raise AnalysisDatabasePrivilegeError("analysis_role_not_found")
    required_true = (
        "bypass_rls",
        "effective_role_safe",
        "login_role_safe",
        ...
        "result_table_access",
        "attempt_table_access",
        "audit_insert_only",
        "forced_rls_present",
    )
    failures = [name for name in required_true if not bool(row[name])]
    if str(row["effective_role"]) != settings.analysis_database_role:
        failures.append("effective_role")
    if bool(row["can_login"]):
        failures.append("role_must_be_nologin")
    if failures:
        raise AnalysisDatabasePrivilegeError(
            "analysis_database_capability_missing:" + ",".join(sorted(failures))
        )
    return AnalysisDatabaseCapability(
        backend="postgresql",
