...
import { PublicProofSwitcher } from "@/components/folynta/localization/public-proof-switcher";
import { HomepageProcessingScene } from "@/components/marketing/homepage-processing-scene";
...
const COPY = {
  en: {
    ...
    proofLabel: "03 · Full-bleed proof",
    proofTitle: "Select the result. Return to the exact source cell.",
    proofBody:
      "One frozen public filing connects source-native XBRL evidence, structured output, metadata, a portable note, and a graph relation.",
    transformationLabel: "04 · Compiler transformation",
    transformationTitle: "Understand. Verify. Connect. Activate.",
    knowledgeLabel: "05 · Knowledge system",
    ...
    qualityLabel: "06 · Measured trust",
...
const TRANSFORMATION = {
  en: [
    [
      "01",
      "Understand",
      "Recover reading order, hierarchy, tables, figures, formulas, and footnotes as typed blocks.",
      "Page → semantic blocks",
    ],
    [
      "02",
      "Verify",
      "Bind claims, numbers, and cells to page, block, bounding box, and immutable receipt.",
      "Block → evidence",
    ],
    [
      "03",
      "Connect",
      "Compile sections into notes, entities, relations, and continuity across documents.",
      "Evidence → knowledge",
    ],
    [
      "04",
      "Activate",
      "Derive Markdown, Vault, RAG JSONL, JSON-LD, and project packs from one verified core.",
      "One core → many uses",
    ],
  ],
  ko: [
    [
      "01",
      "이해",
      "읽기 순서, 계층, 표, 그림, 수식과 각주를 타입이 있는 블록으로 복원합니다.",
      "페이지 → 의미 블록",
    ],
    [
      "02",
      "검증",
      "주장, 숫자와 셀을 페이지, 블록, 바운딩 박스와 불변 영수증에 결합합니다.",
      "블록 → 근거",
    ],
    [
      "03",
      "연결",
      "섹션을 노트, 엔티티, 관계와 문서 간 연속성으로 컴파일합니다.",
      "근거 → 지식",
    ],
    [
      "04",
      "활성화",
      "하나의 검증 코어에서 Markdown, Vault, RAG JSONL, JSON-LD와 프로젝트 팩을 파생합니다.",
      "하나의 코어 → 여러 활용",
    ],
  ],
} as const;
...
export function MarketingLanding({
  ...
  const chapters = TRANSFORMATION[locale].map(
    ([number, title, body, signal]) => ({
      id: `compiler-${number}`,
      number,
      title,
      body,
  ...
  return (
    <StructaraMarketingShell showFooterCta={false}>
      <main id="main-content" className="st-home folynta-home">
        <section
          className="st-home-hero folynta-scene"
          data-scene="01-hero"
          data-truth-class="deterministic-first-party-brand-t1"
        >
          <div className="st-home-copy">
            ...
            <div className="st-actions">
              ...
              <Link href="#proof" className="st-text-action">
                {copy.secondary}
              </Link>
            </div>
        ...
        <HomepageProcessingScene locale={locale} />

        <section
          id="proof"
          className="folynta-proof-scene folynta-scene"
          data-scene="03-proof"
          data-truth-class="registered-public-filing-fixture-t0"
        >
          ...
          <PublicProofSwitcher locale={locale} />
        </section>

        <TransformationStory locale={locale} chapters={chapters} />

        <section
          className="folynta-knowledge-scene folynta-scene"
          data-scene="05-knowledge"
          data-truth-class="source-linked-public-fixture-projection-t1"
        >
          <header className="folynta-section-heading">
            <p>{copy.knowledgeLabel}</p>
            <h2>{copy.knowledgeTitle}</h2>
            <span>{copy.knowledgeBody}</span>
          </header>
          <div className="folynta-knowledge-grid">
            ...
            <article>
              ...
              <span>
                {revenue.current} {DART_PUBLIC_FIXTURE.unit}
              </span>
              <code>receipt: {DART_PUBLIC_FIXTURE.receiptNumber}</code>
            </article>
            <article className="folynta-graph">
              ...
              <code>evidence: source line {revenue.sourceLine}</code>
            </article>
            <article>
              <small>EXPORT</small>
              <strong>Verified core</strong>
              <span>Markdown · Vault · RAG JSONL</span>
              <span>JSON-LD · Knowledge package</span>
            </article>
          </div>
        </section>

        <section
          className="folynta-trust-scene folynta-scene"
          data-scene="06-trust-security"
          data-truth-class="measured-shadow-subset-and-control-diagram"
        >
          <div className="folynta-quality-scene">
            <header className="folynta-section-heading">
              <p>{copy.qualityLabel}</p>
              <h2>{copy.qualityTitle}</h2>
              <span>{copy.qualityBody}</span>
            </header>
            <div
              className="folynta-benchmark-table"
              role="table"
              aria-label={copy.qualityTitle}
            >
              <div role="row">
                <strong role="columnheader">Candidate</strong>
                <strong role="columnheader">Text 1−edit</strong>
                <strong role="columnheader">Table TEDS</strong>
                <strong role="columnheader">Runtime</strong>
                <strong role="columnheader">Est. cost/page</strong>
              </div>
              {candidates.map((dataset) => (
                <div role="row" key={dataset.id}>
                  <span role="cell">{dataset.label}</span>
                  <span role="cell">
                    {formatBenchmarkPercent(
                      dataset.metrics.text_edit_companion,
                    )}
                  </span>
                  <span role="cell">
                    {formatBenchmarkPercent(dataset.metrics.table_teds)}
                  </span>
                  <span role="cell">
            ...
            <div className="folynta-quality-footer">
              <span>
                SHADOW · 18-page fixed subset · RTX 4090 · $0.69/h · no public
                parser promoted
              </span>
              <Link href="/benchmarks">{copy.benchmark}</Link>
            </div>
          </div>

          <div className="folynta-security-scene">
            <header className="folynta-section-heading">
              <p>{copy.securityLabel}</p>
              <h2>{copy.securityTitle}</h2>
              <span>{copy.securityBody}</span>
            </header>
            <div className="folynta-security-layout">
              <div
                className="folynta-security-flow"
                aria-label="Verified document processing flow"
              >
                {[
                  ["Browser", "Local manifest"],
                  ["Quarantine", "Hash + scan"],
                  ["Verified Source", "Immutable receipt"],
                  ["Isolated Worker", "Tenant-scoped"],
                  ["Derived Knowledge", "Source-linked"],
              ...
              <aside>
                ...
                <strong>POLICY RAIL</strong>
                {copy.policyRail.map((policy) => (
                  <span key={policy}>
                    {policy}
        ...
        <section
          className="st-home-final folynta-scene"
          data-scene="07-final"
          data-truth-class="functional-navigation-no-evidence-claim"
        >
          <p>{copy.finalLabel}</p>
          ...
          <small>
            <CheckCircle size={14} aria-hidden="true" /> {PUBLIC_BRAND.tagline}
          </small>
        </section>
      </main>
