export type FolyntaSection = {
  title: string;
  body: string;
  items?: readonly string[];
};

export type FolyntaPage = {
  path: string;
  family:
  ...
  intro: string;
  thesis: string;
  sections: readonly FolyntaSection[];
...
const page = (
  path: string,
  family: FolyntaPage["family"],
  label: string,
  title: string,
  intro: string,
  thesis: string,
  sections: readonly FolyntaSection[],
  primaryAction: FolyntaPage["primaryAction"] = {
    label: "Build your knowledge",
    href: "/signup",
  },
  secondaryAction?: FolyntaPage["secondaryAction"],
): FolyntaPage => ({
  path,
  family,
  label,
...
export const PUBLIC_PAGES: Record<string, FolyntaPage> = Object.fromEntries(
  [
    page(
      "/product",
      "product",
    ...
    page(
      ...
      [
        {
          title: "2026.07 — FOLYNTA foundation",
