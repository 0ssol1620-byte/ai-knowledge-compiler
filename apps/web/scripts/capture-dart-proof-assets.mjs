...
async function prepare(viewport, reducedMotion = "reduce") {
  ...
  await page.addStyleTag({
    content:
      "html{scroll-behavior:auto!important}*{caret-color:transparent!important}.nextjs-toast,.tv-header,.skip-link,nextjs-portal{display:none!important}",
  });
  return { context, page };
}
...
async function captureTabs() {
  ...
  const proof = page.locator(".tv-route-proof");
  await proof.scrollIntoViewIfNeeded();
  for (const tab of ["Original", "Markdown", "Vault", "Graph", "Proof"]) {
    await page.getByRole("tab", { name: tab }).click();
    await page.waitForTimeout(100);
    const file = `TAV-PROOF-T0-DART-EN-${tab.toUpperCase()}-1440x900-v01.webp`;
  ...
  for (const [state, selector] of [
    ["SOURCE-CELL", ".tv-proof-source"],
    ["EVIDENCE-RECEIPT", ".tv-proof-evidence"],
  ]) {
    const file = `TAV-PROOF-T0-DART-EN-${state}-1440x900-v01.webp`;
...
async function captureViewport(name, viewport) {
  ...
  const file = `TAV-PROOF-T0-DART-EN-${name}-${viewport.width}x${viewport.height}-v01.webp`;
  await proof.screenshot({
    path: resolve(outputRoot, file),
    type: "webp",
    quality: 84,
...
async function captureRoute() {
  ...
  const { context, page } = await prepare(viewport);
  const file = "TAV-PROOF-T0-DART-EN-ROUTE-1920x1080-v01.webp";
