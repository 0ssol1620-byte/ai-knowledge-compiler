...
if [[ $# -lt 3 || $# -gt 4 ]]; then
  echo "usage: remote_run_operational_retry.sh WORKER_INDEX INPUT_ROOT OUTPUT_ROOT [--resume-interrupted]" >&2
  exit 2
fi
...
if [[ ${4:-} == "--resume-interrupted" ]]; then
  resume_interrupted=true
elif [[ $# -eq 4 ]]; then
  echo "unsupported fourth argument: $4" >&2
  exit 2
fi
...
if [[ -e "$output_root" && "$resume_interrupted" != true ]]; then
  echo "operational retry output already exists: $output_root" >&2
  exit 2
fi
if [[ ! -x "$python_bin" || ! -f "$runner" || ! -d "$input_root" ]]; then
  echo "operational retry runtime or input is missing" >&2
  exit 2
fi
...
if [[ "$resume_interrupted" == true ]]; then
  printf '{"event":"retry_worker_resumed_after_interruption","worker_index":%s,"unix":%s}\n' \
    "$worker_index" "$(date -u +%s)" >>"$state"
else
  printf '{"event":"retry_worker_started","worker_index":%s,"unix":%s}\n' \
    "$worker_index" "$(date -u +%s)" >>"$state"
fi
...
for suite in parsebench omnidocbench olmocr-bench; do
  ...
  if ! [[ "$expected" =~ ^[1-9][0-9]*$ ]]; then
    echo "invalid retry input count for $suite" >&2
    exit 2
  fi
  printf '{"event":"retry_suite_started","suite":"%s","input_count":%s,"unix":%s}\n' \
    "$suite" "$expected" "$(date -u +%s)" >>"$state"
  resume_args=()
  if [[ "$resume_interrupted" == true && -d "$output_root/$suite" ]]; then
  ...
  printf '{"event":"retry_suite_completed","suite":"%s","unix":%s}\n' \
    "$suite" "$(date -u +%s)" >>"$state"
done

printf '{"event":"retry_worker_completed","worker_index":%s,"unix":%s}\n' \
  "$worker_index" "$(date -u +%s)" >>"$state"
