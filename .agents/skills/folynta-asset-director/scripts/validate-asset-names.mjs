...
const canonical =
  /^FLY-[A-Z0-9-]+-T[0-4]-[A-Z0-9-]+-(?:EN|KO|MULTI)-[A-Z0-9-]*\d+x\d+-v\d{2}\.(?:avif|mp4|png|webm|webp)$/;
const failures = [];
let checked = 0;
...
async function walk(directory) {
  for (const entry of await readdir(directory, { withFileTypes: true })) {
    ...
    checked += 1;
    if (entry.name.startsWith("FLY-") && !canonical.test(entry.name)) {
...
console.log(`FOLYNTA asset names verified (${checked} files).`);
