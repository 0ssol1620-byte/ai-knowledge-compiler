...
const canonical =
  /^STR-[A-Z0-9-]+-T[0-4]-[A-Z0-9-]+-(?:EN|KO|MULTI)-[A-Z0-9-]*\d+x\d+-v\d{2}\.(?:avif|mp4|png|webm|webp)$/;
const failures = [];
let checked = 0;
...
async function walk(directory) {
  for (const entry of await readdir(directory, { withFileTypes: true })) {
    ...
    checked += 1;
    if (entry.name.startsWith("STR-") && !canonical.test(entry.name)) {
...
console.log(`Structara asset names verified (${checked} files).`);
