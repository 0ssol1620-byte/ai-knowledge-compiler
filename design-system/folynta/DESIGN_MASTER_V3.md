...
| 대체 대상 | `FOLYNTA_DESIGN_MASTER_V2_FACING_PAGES_KO` 전문 |
...
| v2의 서술 | 실제 |
| --- | --- |
| 제품명 FOLYNTA | 코드 전체가 **Structara**. `folynta` 문자열 0건. FOLYNTA는 미병합 브랜치에만 |
...
| 레거시 CSS 제거 순서에 `folynta.css` | 없는 파일. 가장 큰 `globals.css`(8,777줄)가 목록에서 누락 |
...
```text
folynta/brand/       ← brand-mark.tsx · structara-glyph.tsx (+ 심볼 신규)
folynta/facing/      ← 신규
folynta/nav/         ← structara-marketing-shell.tsx 의 header/footer 분리
folynta/hero/        ← structara-hero.tsx (+ webgl-scene, G-C 종속)
folynta/theater/     ← 신규
folynta/proof/       ← structara-proof-demo.tsx + workspace/source-viewer.tsx 공용부
folynta/knowledge/   ← knowledge-studio.tsx 분해
folynta/trust/       ← 신규
folynta/instrument/  ← app-shell.tsx + workspace/* 패널
...
대상은 5개 파일 **16,048줄**이다. v2의 목록(`enterprise-refresh → structara* → folynta.css`)은 틀렸다 — `folynta.css`는 `main`에 없고, 가장 큰 두 파일이 빠져 있었다.
...
```text
□ G-A  제품명 확정 — FOLYNTA / Structara                     §0.5
...
코드      0ssol1620-byte/ai-knowledge-compiler main@7ac5098 apps/web 전수
          + origin/agent/folynta-* 4개 브랜치 (FOLYNTA 자산 소재 확인)
...
| 레거시 CSS 목록 | 3개 (`folynta.css` 포함, 없는 파일) | **5개 16,048줄**, 적재 순서·경쟁 선택자 명시 |
